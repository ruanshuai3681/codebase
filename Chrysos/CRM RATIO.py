...
def scrape_crm_data(session, max_unit):
    """Scrape CRM data"""
    df = pd.DataFrame(columns=["jar_barcode", "CRM_type", "gross_mass", "fill"])
    crm_list = list(range(7482, 5300, -1))
    for i in crm_list:
        ...
        try:
            ...
            if 'checked' in retired_str:
                df = df._append({"jar_barcode": jar_barcode, "CRM_type": jar_CRM, "gross_mass": jar_gross_mass, "fill": jar_fill}, ignore_index=True)
...
def main():
    ...
    print(df)
    df.to_csv(r"C:\Users\shuai.ruan\PycharmProjects\shuai_ruan\Web scraping\crms.csv")
